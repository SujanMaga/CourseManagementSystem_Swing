...
//	This class adds instructor detail to the instructor table when executed with randomly generated 8 digit password which can be known from the administrator (admin)
	
	private JLabel heading, bgg, label1, label2, label3, label4, label5, label6,label7;
	...
	AddInstructor(){
		...
		heading = new JLabel("Instructor Registration Form");
		heading.setForeground(Color.ORANGE);
		heading.setBackground(Color.WHITE);
		heading.setFont(new Font("Juice ITC", Font.BOLD, 77));
		heading.setBounds(100, 0, 700, 87);
		add(heading);
...
//		following JCheckBox objects keep the record of teaching section with "0" value in database when unchecked and value ("A","B",...) when checked
...
//		external jar file is required
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == submit){
            try{
            	ConDb c2 = new ConDb();
//            	Random 8 digits password is assigned that can be known from the admin
...
//        		Query to insert the data in instructor table (section1 through section6 are filled with value either "0" or "A","B"....)
            	String q = "insert into instructor(name,pw, address, course, phone, email, dob,section1,section2,section3,section4,section5,section6) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
            	
//            	using PreparedStatement interface to execute query
            	PreparedStatement pst = c2.connection.prepareStatement(q);
            	
            	String u = "",v = "",w="",x="",y="",z ="";
            	
//            	"0" or "A","B".. value in section1,section2..... column based on unchecked or checked
            	if(sec1.isSelected()) {
            		u += sec1.getText();
            	}
            	else {
...
//            	specifying the parameter in the query 
            	pst.setString(1, a);
