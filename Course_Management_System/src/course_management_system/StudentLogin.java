...
//	            verifying the credentials with the record in the database
	            String q = "select * from student where id='"+i+"' and pw='"+p+"'";
	            
	            ResultSet rs = c1.statement.executeQuery(q); 
	            if(rs.next()){
//	            	taking the value of id to view results and instructor along with optionals detail to choose 
	            	unique = i;
	                new StudentTab();
	                setVisible(false);
	            }else{
